...
async function main() {
  try {
    // 定义图片路径
    const imagePath = path.join(__dirname, '329193.JPG');
    ...
    // 上传图片
    const objectName = 'photographer.jpg'; // 在OSS中保存的文件名
    const result = await uploadToOSS(imagePath, objectName);
    
    if (result.success) {
      console.log('图片上传成功，URL:', result.url);
      console.log('请在 about.html 中使用下面的URL替换现有的摄影师照片:');
      console.log(`http://${OSS_CONFIG.bucket}.${OSS_CONFIG.region}.aliyuncs.com/${OSS_CONFIG.prefix}${objectName}`);
    } else {
